...
class GiftBizDataModule {
    ...
    @Bean
    fun giftBizWebClient(
        webClientBuilder: WebClient.Builder,
        serviceConfig: GiftBizServiceConfig,
    ): WebClient {
        val config = serviceConfig.configs.first { it.id == GiftBizServiceConfig.Config.ServiceID.GIFTBIZ }
        return buildWebClient(webClientBuilder, config)
    }

    private fun buildWebClient(
        webClientBuilder: WebClient.Builder,
        config: GiftBizServiceConfig.Config,
